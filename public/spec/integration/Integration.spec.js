...
define([
    ...
    describe("Application (Integration Test)", function () {
        ...
        var getFirstGameView = function() {
            return view.gameCollectionView && view.gameCollectionView.children.findByIndex(0);
        };
        ...
        describe("happy path", function() {
            ...
            it('picks a game matching criteria', function (done) {
                ...
                waitFor(function() {
                    if (getFirstGameView()) {
                        var correctGame = getFirstGameView().$el.find('h1').eq(1).text().trim() === 'c game';
            ...
            it('shows another game after closing and changing criteria', function (done) {
                ...
                waitFor(function() {
                    if (getFirstGameView()) {
                        var correctGame = getFirstGameView().$el.find('h1').eq(1).text().trim() === 'a game';
                        var filteringWorks = view.gameCollection.getFilteredCollection(view.criteria).length === 1;
                        return correctGame && filteringWorks;
                    }
                }, done);
            ...
            it('shows a new game from a new user\'s collection', function (done) {
                ...
                waitFor(function() {
                    if (getFirstGameView()) {
                        var gameShowing = getFirstGameView().$el.find('h1').eq(1).text().trim() === 'e game';
                        var filteringWorks = view.gameCollection.getFilteredCollection(view.criteria).length === 1;
        ...
        describe("User submits before collection xhr finishes", function() {
            ...
            it('shows game as soon as xhr finishes', function (done) {
                ...
                waitFor(function() {
                    if (getFirstGameView()) {
                        return getFirstGameView().$el.find('h1').eq(1).text().trim().indexOf('game') > -1;
                    }
                }, done);
            });
        });
